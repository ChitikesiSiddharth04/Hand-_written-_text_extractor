...
def recognize_text_trocr(roi_image):
    ...
    # Set parameters to obtain scores
    ...
    token_scores = generated_ids.scores  # List of tensors with scores for each generated token
    # print(token_scores)

    # Calculate confidence scores for each token
    confidence_scores = []

    # Iterate through the generated scores
    for i, score_tensor in enumerate(token_scores):
        # Get the predicted token ID for the i-th token
        # print(score_tensor.shape)
        predicted_token_id = torch.argmax(score_tensor, dim=-1)  # Shape: (vocab_size,)
        # print(predicted_token_id)
        
        if 0 <= predicted_token_id < score_tensor.size(1):  # Check against the score tensor size
            # Apply softmax to get probabilities and then access the token probability
            token_probs = torch.nn.functional.softmax(score_tensor, dim=-1)
            token_prob = token_probs[0, predicted_token_id].item()  # Get probability for the predicted token
        else:
            token_prob = 0.0  # Handle invalid token ID
            print(f"Invalid token ID: {predicted_token_id}, Current score tensor size: {score_tensor.size(1)}")
        # Get the probability for the predicted token
        # token_prob = torch.nn.functional.softmax(score_tensor, dim=-1)[predicted_token_id].item()
...
def recognize_text_trocr_2(roi_image):
    ...
    for i, score_tensor in enumerate(token_scores):
        ...
        # Check the shape of predicted_token_id
        if 0 <= predicted_token_id < score_tensor.size(1):  # Check against the score tensor size
            # Apply softmax to get probabilities and then access the token probability
            token_probs = torch.nn.functional.softmax(score_tensor, dim=-1)
            token_prob = token_probs[0, predicted_token_id].item()  # Get probability for the predicted token
...
print(df)
df.to_excel("Prob.xlsx",index=False)
